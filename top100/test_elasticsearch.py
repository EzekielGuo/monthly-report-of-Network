...
@desc:之前跟网络部沟通协调后，在 elkc.21vianet.com 中筛掉了绝大多数无用日志，于是有可能某设备某端口一个月内也不会有一条日志出现在elk的情况，此时应判定端口可用率为 100% 或 0% ；
    脚本原理：查询本月所有该端口down的时间戳，放在一个列表中，根据每个时间戳继续轮询该端口up的时间戳，取uptime>=downtime的部分，放在一个列表中排序，取第0个，计算差值，单位为s。
'''
...
def elastic(ip,port,index):
    ## 创建连接
    es = Elasticsearch([{'host': 'xx.xx.xx.74', 'port': 9200, 'timeout': 10000}])
    ## 闪断次数
    flash = 0
    ## 端口总中断时间
    duration_all = 0
